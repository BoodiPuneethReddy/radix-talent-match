...
export default function SkillMatchingPage() {
  const [status, setStatus] = useState({ code: 501, message: 'Skill Matching module is under development.' })

  const loadStatus = async () => {
    try {
      await moduleService.getSkillMatchingStatus()
    } catch (e) {
      setStatus({
        code: e.response?.status || 501,
        message: e.response?.data?.detail || 'Skill Matching module is under development.'
      })
    }
  }
